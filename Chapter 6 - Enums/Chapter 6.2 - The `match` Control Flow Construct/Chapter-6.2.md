...
In the match expression for this code, we add a variable called `state` to the pattern that matches values of the variant `Coin::Quarter`. When a `Coin::Quarter` matches, the `state` variable will bind to the value of the quarter's state. Then, we can use `state` in the code for that arm, like so:
...
Now let's consider the second call of `plus_on` in Listing 6-5, where `x` is `None`. We enter the `match` and compare to the first arm:
